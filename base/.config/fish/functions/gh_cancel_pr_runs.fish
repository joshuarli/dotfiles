function gh_cancel_pr_runs --description "Cancel all active CI runs for the current branch's PR"
    ...
    set -l pr_args
    if test -n "$branch"
        set -a pr_args $branch
    end
    if test -n "$_flag_repo"
        set -a pr_args --repo $repo
    end

    set sha (gh pr view $pr_args --json headRefOid --jq '.headRefOid' 2>/dev/null)
    if test -z "$sha"
        echo "error: no PR found for $(test -n "$branch"; and echo "$branch"; or echo "current branch") in $repo" >&2
        return 1
    end
